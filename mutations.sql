...
-- A: To satisfy this requirement I would create a new table named employees_categories which will
--store the relationship between employees and categories. The "employee_id" column will store the ID 
--of the employee and the "category_id" column will store the ID of the category that the employee is 
--an expert in. The primary key of the table will be a tuple of (employee_id, category_id) pairs, 
--which ensures that each combination of employee and category is unique. By inserting records into 
--this table, you can associate employees with categories and keep track of who is an expert in what.
-- Q: What type of relationship is this? (e.g. 1-1, 1-many, or many-to-many?)
-- A: This is many-to-many relationship. An employee can be an expert in zero or more categories 
--and a category may have zero or more experts in it.
...
WHERE e.office = 'London' AND c.category_name ='Dairy Products';
